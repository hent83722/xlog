...
Zyrnix supports multiple sinks out of the box. In addition to console and file sinks, this release adds two additional, commonly-requested backends:
...
```
auto logger = std::make_shared<Zyrnix::Logger>("my_logger");
auto udp = std::make_shared<Zyrnix::UdpSink>("127.0.0.1", 5140);
auto sys = std::make_shared<Zyrnix::SyslogSink>("myapp", LOG_PID, LOG_USER);
logger->add_sink(udp);
logger->add_sink(sys);
